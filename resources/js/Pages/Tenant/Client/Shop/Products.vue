<template>
  <ClientLayout :title="t('common.all_products')">
    <div class="max-w-7xl mx-auto px-4 py-8">
      <!-- Breadcrumbs -->
      <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <Link :href="route('tenant.shop')" class="hover:theme-primary">{{ t('common.shop') }}</Link>
        <span>/</span>
        <span class="text-gray-900 font-medium">{{ t('common.all_products') }}</span>
      </nav>

      <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ t('common.all_products') }}</h1>
        ...
        <aside class="hidden lg:block w-60 flex-shrink-0">
          <div class="bg-white rounded-2xl border border-gray-100 p-5 space-y-6 sticky top-4">
            ...
            <div v-if="categories.length">
              <p class="text-xs font-semibold text-gray-500 uppercase mb-2">{{ t('common.category') }}</p>
              <div class="space-y-1">
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="localFilters.category" value="" class="theme-primary-accent" />
                  {{ t('common.all') }}
                </label>
                <label v-for="cat in categories" :key="cat.id" class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="localFilters.category" :value="cat.slug" class="theme-primary-accent" />
                  {{ cat.name }}
                </label>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-1">
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="localFilters.type" value="" class="theme-primary-accent" />
                  {{ t('common.all') }}
                </label>
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="localFilters.type" value="physical" class="theme-primary-accent" />
                  {{ t('client.shop.category.physical') }}
                </label>
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="localFilters.type" value="digital" class="theme-primary-accent" />
            ...
            <!-- In stock -->
            <label class="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" v-model="localFilters.in_stock" class="theme-primary-accent" />
              {{ t('common.in_stock_only') }}
            </label>

            <button
              @click="applyFilters"
              class="w-full theme-primary-bg text-white py-2 rounded-xl text-sm font-semibold hover:opacity-90 transition"
            >
        ...
        <div class="flex-1">
          ...
          <div v-if="products.last_page > 1" class="mt-10 flex justify-center gap-2">
            <Link
              ...
              :class="
                page.active
                  ? 'theme-primary-bg text-white theme-primary-border'
        ...
        <div class="absolute inset-y-0 right-0 w-full max-w-xs bg-white shadow-xl flex flex-col">
          ...
          <div class="p-5 border-t">
            <button
              ...
              class="w-full theme-primary-bg text-white py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 transition"
            >
              {{ t('client.shop.category.apply_filters') }}
            </button>
          </div>
...
function applyFilters() {
  router.get(route('tenant.shop.products'), localFilters, { preserveScroll: true })
}
</script>
