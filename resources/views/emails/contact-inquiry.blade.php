...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
    <h2 style="color: #1d4ed8; border-bottom: 2px solid #1d4ed8; padding-bottom: 8px;">
        {{ __('mail.enquiry_from_landing') }}
    </h2>

    <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
        <tr>
            <td style="padding: 8px 0; font-weight: bold; width: 120px; color: #555;">{{ __('mail.label_full_name') }}</td>
    ...
    <div style="margin-top: 20px;">
        <p style="font-weight: bold; color: #555; margin-bottom: 8px;">{{ __('mail.label_message') }}</p>
    ...
    <p style="margin-top: 24px; color: #6b7280; font-size: 12px; border-top: 1px solid #e5e7eb; padding-top: 12px;">
        {{ __('mail.sent_at', ['when' => $inquiry->created_at->format('d.m.Y H:i')]) }} &nbsp;·&nbsp;
        <a href="{{ url('/admin/contacts') }}" style="color: #1d4ed8;">{{ __('mail.open_in_admin') }}</a>
    </p>
</body>
</html>
